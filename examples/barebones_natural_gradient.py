"""Minimal natural-gradient adaptation for the double geometric distribution.

Unlike the original INES, this variant has no evolution path and does not
floor the Fisher denominator with max(Var[|Z|], 1). Its only adaptation
hyperparameter is eta.
"""

from __future__ import annotations

import numpy as np


def sample_double_geometric(
    rng: np.random.Generator, delta: np.ndarray, population_size: int
) -> np.ndarray:
    q = delta / (np.sqrt(1.0 + delta**2) + 1.0)
    p = np.clip(1.0 - q, 1e-12, 1.0 - 1e-12)
    log_q = np.log1p(-p)
    shape = (len(delta), population_size)
    g1 = np.floor(np.log1p(-rng.random(shape)) / log_q).astype(int)
    g2 = np.floor(np.log1p(-rng.random(shape)) / log_q).astype(int)
    return g1 - g2


class BarebonesNaturalGradientINES:
    """Path-free `(1, lambda)` DG natural-gradient optimizer."""

    def __init__(
        self,
        x0: np.ndarray,
        delta0: float,
        eta: float,
        population_size: int = 10,
        seed: int | None = None,
        binary: bool = False,
    ) -> None:
        self.rng = np.random.default_rng(seed)
        self.x = np.asarray(x0, dtype=int).reshape(-1, 1).copy()
        self.n = len(self.x)
        self.delta = np.full((self.n, 1), delta0, dtype=float)
        self.eta = eta
        self.population_size = population_size
        self.binary = binary

    def ask(self) -> np.ndarray:
        self.steps = sample_double_geometric(self.rng, self.delta, self.population_size)
        candidates = self.x + self.steps
        return candidates & 1 if self.binary else candidates

    def tell(self, values: np.ndarray) -> None:
        best = int(np.argmin(values))
        selected_step = self.steps[:, best, None]
        self.x += selected_step

        # E[|Z|] = delta and Var(|Z|) is the Fisher information for log(q).
        fisher = self.delta * np.sqrt(1.0 + self.delta**2)
        natural_gradient = (np.abs(selected_step) - self.delta) / fisher
        self.delta *= np.exp(self.eta * natural_gradient)


if __name__ == "__main__":
    target = np.array([3, -2, 7])[:, None]
    optimizer = BarebonesNaturalGradientINES(
        np.zeros(3, dtype=int), delta0=2.0, eta=0.1, seed=1
    )
    for _ in range(1_000):
        points = optimizer.ask()
        fitness = np.sum((points - target) ** 2, axis=0)
        optimizer.tell(fitness)
        if fitness.min() == 0:
            break
    print("x =", optimizer.x.ravel())
    print("delta =", optimizer.delta.ravel())
