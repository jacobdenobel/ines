"""Single-file path-free DG natural gradient and paper-style runner.

Unlike original INES, this variant has no evolution path and does not floor
the Fisher denominator with ``max(Var[|Z|], 1)``. Its only adaptation
hyperparameter is ``eta``. Running this file directly evaluates it on one of
the four quadratic functions from the paper and saves objective- and
delta-history plots.
"""
...
class BarebonesNaturalGradientINES:
    """Path-free ``(1, lambda)`` DG natural-gradient optimizer."""
    ...
    def tell(self, values: np.ndarray) -> None:
        ...
        # E[|Z|] = delta and Var(|Z|) is the Fisher information for log(q).
...
def run_paper_benchmark(
    ...
    """Run the path-free variant under the paper's quadratic conditions."""
...
def main() -> None:
    ...
    stem = f"natural_gradient_{args.function}_{args.dimension}d"
    ...
    effective_eta = (
        args.eta if args.eta is not None else (2.0 / args.dimension) ** (1.0 / 3.0)
    )
    print("eta =", effective_eta)
